...
def chunk_md(
    ...
    try:
        ...
        logger.error("'unstructured' library not found. Please install it to use chunk_md. Try: pip install unstructured")
        print("Error: 'unstructured' library not found. Please install it. Try: pip install unstructured")
    except Exception as e:
...
def ask(
    ...
    model: Annotated[
        str, 
        typer.Option(
            help="Model to use: openai, claude, llama, google, etc."
        )
    ] = "openai",
):
    """Ask a question about Igor's blog content."""
    if debug:
        logger.info(f"CLI 'ask' called with: question='{question}', facts={facts}, model='{model}', debug={debug}")
    try:
...
def where(
    ...
    if debug:
        logger.info(f"CLI 'where' called with: topic='{topic}', num_docs={num_docs}, model='{model}', debug={debug}")
    try:
        response = asyncio.run(askig_logic.iask_where_logic(topic, num_docs, debug, model))
        print(response)
    except Exception as e:
...
def versions():
    ...
    libraries = ["typer", "click", "rich", "langchain", "fastmcp", "openai", "pydantic", "loguru", "uvicorn"] # Removed fastapi, added uvicorn as it's in askig_server
    ...
    print("Custom modules: askig_logic, askig_models, askig_server, askig_build_logic (version not applicable)")
