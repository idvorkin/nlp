...
image = (
    ...
    Image.from_dockerhub("python:3.10-slim-bullseye")
    .apt_install("git")
...
class AlpacaLoRAModel:
    ...
    def generate(self, instructions: list[str]):
        for instrctn in instructions:
            rich.rich_print(f"[light blue] Instruction: {instrctn}")
            response = self.evaluate(instrctn)
            rich.rich_print(f"Response {respone}")
            print()
...
def main():
    ...
    while True:
        instruct = input("Give me something to say:")
        model.generate.call([instruct])
