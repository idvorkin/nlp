...
DEFAULT_FAISS_INDEX_DIR = FAISS_INDEX_NAME  # Relative to where this script/module is run
...
g_all_documents = None  # To store all docs from FAISS, reducing .get() calls
...
def get_faiss_db():
    ...
    # This assumes iwhere.py (and its 'build' command) creates the index in DEFAULT_FAISS_INDEX_DIR
    # or the user ensures it's in ALTERNATE_FAISS_INDEX_DIR.
    if os.path.exists(DEFAULT_FAISS_INDEX_DIR):
        db_dir = DEFAULT_FAISS_INDEX_DIR
    elif os.path.exists(ALTERNATE_FAISS_INDEX_DIR):
        logger.info(
            f"Using alternate blog index location: {ALTERNATE_FAISS_INDEX_DIR}"
        )
        ...
        raise Exception(
            f"Blog index not found in {DEFAULT_FAISS_INDEX_DIR} or {ALTERNATE_FAISS_INDEX_DIR}. "
            "Please ensure the index is built and accessible."
        )

    logger.info(f"Loading FAISS index from: {db_dir}")
    g_blog_content_db = FAISS.load_local(
        db_dir, embeddings, allow_dangerous_deserialization=True
    )
    # Fetch all documents once and store them if db.get() is expensive
    # This might consume a lot of memory for very large indexes.
    logger.info("Fetching all documents from FAISS index into memory...")
...
async def iask_where_logic(
    ...
    timing.end_db_load()
    if db is None:
        raise Exception("Blog index (FAISS) not loaded.")
